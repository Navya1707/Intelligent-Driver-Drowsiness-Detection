    ...
    def distracted_driving_alert(self):
        ...
        if detected_cigarettes:
            ...
            if not self.alarm_status:
                self.alarm_status = True
                t = Thread(target=self.alarm, args=(
                    'Avoid smoking while driving please!',))
                t.daemon = True
                t.start()
        ...
        if detected_food:
            ...
            if not self.alarm_status:
                self.alarm_status = True
                t = Thread(target=self.alarm, args=(
                    'Avoid eating while driving please!',))
                t.daemon = True
                t.start()

    def start_detection(self):
        if self.vs is None:
            logging.error("Video is not initialized.")
            return
            ...
            for rect in rects:
                ...
                if distance > self.YAWN_THRESH:
                    ...
                    if not self.alarm_status2 and not self.saying:
                        ...
                        t = Thread(target=self.alarm, args=(
                            'Please get some fresh air or have coffee',))
                        t.daemon = True
                        t.start()
                else:
                    self.alarm_status2 = False
...
def main():
    parser = argparse.ArgumentParser(
        description="Driver drownsiness detection")
